...
class SihaHeartRateIntradayDataset(SihaDataset):
    ...
    class Default:  # pylint: disable=too-few-public-methods
        ...
        PIPELINE = ProcessingPipeline([
            ...
            ResampleOperator(rule='15min', offset='00h00min', on='time', aggregation_definition='mean'),
            ResetIndexOperator(),
        ])
        ...
        PIPELINE = ProcessingPipeline([
            ...
            ComposeOperator([
                ...
                SequenceOperator([
                    ...
                    ApplyOperator(lambda l: [x.split()[0].strip() for x in l if x.strip()]),
                ])
            ])
        ])
...
class SihaCompositeDataset:
    ...
    class Default: #pylint: disable=too-few-public-methods
        ...
        @staticmethod
        def drop_days_below_min_steps(dataframe, min_steps=1000, time_col="time", pid_col="patientID"):
            ...
            pipeline = ProcessingPipeline([
                ...
                SetFeaturesValueOperator(selector=lambda df: ~df.index.isin(days_to_drop)),
        ...
        def create_data_availability_dict(dataframe):
            ...
            """
            tmp = {}
            tmp["before"] = dataframe[(dataframe["Ramadan"] == 0)]["time"].dt.floor("d").unique().shape[0]
            tmp["during"] = dataframe[(dataframe["Ramadan"] == 1)]["time"].dt.floor("d").unique().shape[0]
            tmp["after"] = dataframe[(dataframe["Ramadan"] == 2)]["time"].dt.floor("d").unique().shape[0]
        ...
        GOOD_AVAILABILITY = SequenceOperator([
            ...
            ApplyOperator(lambda group, fun=create_data_availability_dict: fun.__func__(group)),
            FunctionOperator(lambda df: df[(df["before"] >= 5) & (df["during"] >= 5)])
        ])
        ...
        PIPELINE = ProcessingPipeline([
            ...
            DropNAOperator(),
            CreateFeatureOperator(feature_name='Ramadan', feature_creator=lambda df, fun=ramadan_flag: fun.__func__(df['time'])),
            CreateFeatureOperator('hypo', lambda df, fun=is_hypo: fun.__func__(df['CGM'])),
            CreateFeatureOperator('hyper', lambda df, fun=is_hyper: fun.__func__(df['CGM'])),
            DropNAOperator(),
            ...
            FunctionOperator(lambda df, fun=drop_days_below_min_steps: fun.__func__(df)),
            ComposeOperator([
                NoopOperator(),
                GOOD_AVAILABILITY,
            ]),
            FunctionOperator(lambda df, good_availability: df[0][df[0]["patientID"].isin(good_availability[0].index.to_list())]),
            LogOperator(function=lambda df: df["patientID"].unique().shape[0], string="Remaining number of participants:"),
            # Analyze data
            ComposeOperator([
                BEFORE_RAMADAN,
                DURING_RAMADAN,
    ...
    def _load_datasets(self):
        ...
        df_intra = SihaHeartRateIntradayDataset(folder=self.profast_folder).processed_dataframe()
        df_calories = SihaCaloriesIntradayDataset(folder=self.profast_folder).processed_dataframe()
        df_steps = SihaStepsIntradayDataset(folder=self.profast_folder).processed_dataframe()
        df_distance = SihaDistanceIntradayDataset(folder=self.profast_folder).processed_dataframe()
        df_cgm = SihaCgmDataset(folder=self.profast_folder).processed_dataframe()
    ...
    def boxplot_correlation(self, dataframe, secondary_col, main_col="CGM", remove_zero_steps=True):
        ...
        sns.stripplot(x='patientID', y=secondary_col, data=df_tmp,

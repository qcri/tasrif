...
class FlagDayIfValidEpochsLargerThanOperator(ValidatorOperator):
    ...
    :return: None
    """
    def __init__(self, max_invalid_minutes_per_day: int = 0, **kwargs):
    ...
    def process(self, *data_frames):
        ...
        for data_frame in data_frames:
            ...
            invalid_epochs = data_frame.groupby(
                [self.experiment_day_col])["_tmp_flag_"].transform(
                    lambda x: x.sum()) > max_invalid_epochs_per_day
            data_frame.loc[
                invalid_epochs,
                self.invalid_col] |= InvCode.FLAG_DAY_TOO_MANY_INVALID_EPOCHS

...
class FlagDayIfDaySumSmallerThanOperator(ValidatorOperator):
    ...
    :return: None
    """
    def __init__(self, col, sum_threshold: int = 0, **kwargs):
    ...
    def process(self, *data_frames):
        ...
        for data_frame in data_frames:
            days_smaller_than_threshold = data_frame.groupby([
                self.pid_col, self.experiment_day_col
            ])[self.col].transform(lambda x: x.sum()) < self.sum_threshold
            data_frame.loc[days_smaller_than_threshold,
                           self.invalid_col] |= InvCode.FLAG_DAY_SHORT_COL_SUM

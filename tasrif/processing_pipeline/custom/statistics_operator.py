...
class StatisticsOperator(ProcessingOperator):
    ...
    def __init__(self, participant_identifier="Id", date_feature_name="Date", filter_features=None):
    ...
    def _compute_statistics(self, data_frame):
        ...
        dt_data_frame[dt_feature] = pd.to_datetime(dt_data_frame[self.date_feature_name])
        result.extend(self._get_min_max_day(dt_data_frame, dt_feature))

        result.append(self._get_overview('participant_vs_features', data_frame, 'mean_days_per_participant'))
        result.append(self._get_overview('date_vs_features', data_frame, 'mean_participants_per_day'))
    ...
    def _get_participant_count(self, data_frame):
        ...
        for column in columns:
            if self.filter_features and column in self.filter_features:
                ...
                count[column] = data_frame[data_frame.apply(lambda x, func=column_map_func, column=column: func(x[column]), axis=1)][self.participant_identifier].nunique()
            else:
                count[column] = data_frame[data_frame[column].notna()][self.participant_identifier].nunique()
    ...
    def _get_duplicate_rows_count(self, data_frame):
        ...
        for column in columns:
            if self.filter_features and column in self.filter_features:
                column_map_func = self.filter_features[column]
                dup_count[column] = data_frame[data_frame.apply(lambda x, func=column_map_func, column=column: func(x[column]), axis=1)][[self.date_feature_name, self.participant_identifier]].duplicated().sum()
            else:
                dup_count[column] = data_frame[data_frame[column].notna()][[self.date_feature_name, self.participant_identifier]].duplicated().sum()

        return dup_count

    # Pylint no-self-use rule is disabled to apply the same style as for in the other functions such as _get_duplicate_rows_count etc..
    ...
    def _get_row_count(self, data_frame):
        ...
        for column in columns:
            if self.filter_features and column in self.filter_features:
                column_map_func = self.filter_features[column]
                row_count[column] = data_frame[data_frame.apply(lambda x, func=column_map_func, column=column: func(x[column]), axis=1)].shape[0]
            else:
    ...
    def _get_min_max_day(self, data_frame, dt_feature):
        ...
        for column in columns:
            if column != dt_feature:
                if self.filter_features and column in self.filter_features:
                    column_map_func = self.filter_features[column]
                    min_day[column] = data_frame[data_frame.apply(lambda x, func=column_map_func, column=column: func(x[column]), axis=1)][dt_feature].min()
                    max_day[column] = data_frame[data_frame.apply(lambda x, func=column_map_func, column=column: func(x[column]), axis=1)][dt_feature].max()
                else:
                    min_day[column] = data_frame[data_frame[column].notna()][dt_feature].min()
                    max_day[column] = data_frame[data_frame[column].notna()][dt_feature].max()
    ...
    def _get_overview(self, overview_type, data_frame, statistic_type):
        ...
        for column in columns:
            skip_column = self.participant_identifier if overview_type == 'participant_vs_features' else self.date_feature_name

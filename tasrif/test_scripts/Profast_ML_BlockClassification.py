...
# %%
experiment_path = "/home/palotti/github/tasrif/data/profast2020/scenario2/"

# %%
signals = ["time", "patientID", "HeartRate", "mets", "Calories", "Steps", "Distance", "CGM"]

winsizes = ["5h"] # ["3h", "4h", "5h"]
deltas = ["0h"] 
...
for param in tqdm(params):
    ...
    if s(experiment_path, winsize, delta, win_of_interest):
        print("%s: %s, %s, %s was already processed" % (experiment_path, winsize, delta, win_of_interest))
        continue

    
        
    df_timeseries, df_labels, df_label_time, df_pids = generate_timeseries_df_from_block(df, signals, 
                                                                                         winsize,
                                                                                         delta,
                                                                                         win_of_interest,
    ...
    data = data.dropna()
    save_data(experiment_path, data, winsize, delta, win_of_interest)
...
data, feature_mapping = prepare_ml(winsize, delta, win_of_interest, signals)

train_data, test_data = split_train_test(data, featset, feature_mapping=feature_mapping)

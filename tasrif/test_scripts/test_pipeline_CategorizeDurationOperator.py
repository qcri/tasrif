...
df = pd.DataFrame()
df["Date"] = dates
df["Last_Date"] = df["Date"].apply(
    lambda x: x
    + timedelta(
        days=np.random.randint(3),
...
operator = CategorizeDurationOperator(
    duration_feature_name="Duration", category_definition="day"
)
...
df3 = df.copy()
operator = CategorizeDurationOperator(
    duration_feature_name="Duration", category_definition=["day", "hour"]
)
...
operator = CategorizeDurationOperator(
    ...
    category_definition=[{"day": "days_difference"}, {"hour": "hours_difference"}],
)

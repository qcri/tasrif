...
base_datasets = SequenceOperator([
    ...
    ComposeOperator([
        ...
        JqOperator('map({patientID} + .data.activities_tracker_steps[].data."activities-tracker-steps"[0])'), # Steps
        JqOperator('map({patientID} + .data.activities_tracker_distance[].data."activities-tracker-distance"[0])'), # Distance
        JqOperator('map({patientID} + .data.activities_tracker_calories[].data."activities-tracker-calories"[0])'), # Calories
    ]),
    FlattenOperator(),
...
df_intra = MergeOperator(on=['patientID', 'dateTime'], how='outer') \
            .process(*df_intra, df_cgm).dropna()
...
feature_creator_pipeline = SequenceOperator([
    ResetIndexOperator(),
    CreateFeatureOperator(feature_name="Ramadan", feature_creator=lambda df: ramadan_flag(df['dateTime'])),
    CreateFeatureOperator(feature_name="hypo", feature_creator=lambda df: df['CGM'] <= 70),
    CreateFeatureOperator(feature_name="hyper", feature_creator=lambda df: df['CGM'] >= 180),
])
...
# %% slideshow={"slide_type": "subslide"}
filter_pipeline = FilterOperator(participant_identifier="patientID",
                                 date_feature_name="dateTime",
                                 day_filter={"column": "Steps", 
...
days_per_patient = SequenceOperator([
    GroupbyOperator(by='patientID'),
    ResampleOperator('D', on='dateTime', aggregation_definition='size'), # Counts number of rows
    ResetIndexOperator(),
    AggregateOperator(
...
good_avail_pipeline = SequenceOperator([
    ...
    ApplyOperator(func=lambda g: (g['dateTime'].iloc[-1] - g['dateTime'].iloc[0]).days),
    UnstackOperator(),
    ...
    FilterOperator(epoch_filter=lambda df: (df["before"] >= 5) & (df["during"] >= 5)),
])
...
create_report(df_ramadan[['patientID', 'HeartRate', 'Calories', 'Steps', 'Distance', 'CGM']])
...
def boxplot_correlation(df, secondary_col, main_col="CGM",
    ...
    pipeline = ProcessingPipeline([
        CreateFeatureOperator(
            feature_name='Date',
            feature_creator=lambda df: df['dateTime'].date()),
...
emr_pipeline = SequenceOperator([
            ...
            CreateFeatureOperator(
                ...
                feature_creator=lambda df: df['Diabetes Medication'].replace("+", ", ").split(",")),
            CreateFeatureOperator(
                feature_name='Diabetes Medication',
                feature_creator=lambda df: [x.split()[0].strip() for x in df['Diabetes Medication'] if x.strip()]),
...
df_agg_pipeline = AggregateOperator(
                      ...
                      aggregation_definition= {'Steps': ['sum', 'mean'],
                                               'Calories': ['sum', 'mean'],
                                               'Distance': ['sum', 'mean'],
                                               'HeartRate': ['mean'],
                                               'CGM': ['mean'],
...
df_intra = EncodeCyclicalFeaturesOperator(date_feature_name="dateTime", 
                                          category_definition='hour').process(df_intra)[0]

...
pipeline = SequenceOperator(
    [
        FitbitIntradayDataset(
            fitbit_intraday_data_folder, table_name="Steps", num_files=5
        ),
        JsonNormalizeOperator(),
        ConvertToDatetimeOperator(
...
try:
    yaml_config_path = os.path.join(
        os.path.dirname(__file__), "yaml_config/steps_dataset.yaml"
    )
except:
    yaml_config_path = "yaml_config/steps_dataset.yaml"
